...
class DatabaseManager:
    ...
    def _rechercher_local(self, nom: str) -> Optional[AlimentNutrition]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            row = conn.execute(query, (f"%{nom}%",)).fetchone()
        if not row:
            return None
        return AlimentNutrition(
            nom=row["alim_nom_fr"],
    ...
    def rechercher_aliment(self, nom: str) -> Optional[AlimentNutrition]:
        """Retourne les données nutritionnelles : SQLite CIQUAL (prioritaire) → API OpenFoodFacts."""
        # CIQUAL local en priorité : données vérifiées et complètes pour les aliments de base
        result = self._rechercher_local(nom)
        if result:
            logger.info("Données CIQUAL trouvées en local pour '%s'", nom)
            return result
        # Fallback API pour les aliments absents de la base locale
